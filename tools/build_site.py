...
RESEARCH=(f'<title>先行研究と、この設計の根拠｜ AI政策くらべ</title>\n'
  ...
  + _rs("keep",
    ...
    + " ／ " + _cite("答弁書（第212回国会 質問第87号）", "https://www.shugiin.go.jp/internet/itdb_shitsumon.nsf/html/shitsumon/b212087.htm"))

  + _rs("keep",
    "AI生成物の表示義務（2026年の公職選挙法改正）に、そもそも該当しない作りにしてある",
    "2026年7月17日に公布された公職選挙法の改正（令和8年法律第58号、施行は2027年3月1日）は、"
    "選挙運動に用いるAI生成物への表示を義務づける規定を新設した。"
    "<b>対象は条文上「画像又は映像」に限られており、テキストや集計は含まれない。</b>罰則も設けられていない。",
    "このサイトは画像も映像も生成していません（本文と表だけです）。"
    "そのうえで、AIをどこにどう使っているかは、この改正より前から公開しています。"
    "<b>義務があるから開示するのではなく、義務の有無にかかわらず開示する</b>という立場です。",
    _cite("e-Gov 法令検索 公職選挙法", "https://laws.e-gov.go.jp/law/325AC1000000100"))
  ...
  + _rs("drop",
    ...
    "日本語の議会発言に対する賛成／反対／中立のスタンス分類は、"
    "<b>NTCIR QA Lab-PoliInfo として10年近く共有タスク化</b>されており、公開データセットと評価指標がある。"
    "一方で、<b>政党名という手がかりだけでモデルのラベルが偏る</b>ことを示した実証研究もある。",
    "AIが判定した立場を、検証なしに有権者へ提示することはしません。"
    "「政策で照らす」の各党の立場は、参議院の採決か各党の公約から人が判断し、"
    "設問ごとに<b>何にもとづく判断かを表示</b>しています。"
    "ただし、既存ベンチマークがある以上「精度を測っていない」で済ませられる状態ではありません（03を参照）。",
  ...
  + _rs("open",
    ...
    "日本語の議会発言に対するスタンス分類には、公開データセットと評価指標を備えた"
    "共有タスク（NTCIR QA Lab-PoliInfo）が存在する。"
    "既存のベンチマークがある領域では、「測っていない」という状態の説明が要る。",
    "このサイトは、AIに立場を自動判定させて表示する作りにはしていません（02を参照）。"
    "ただし発言の抽出・分野への割り当てにはAIが関与しています。"
    "<b>その工程の精度を、公開ベンチマークで測ってはいません。</b>"
    "現状は全件の引用を原文と機械照合することで担保していますが、これは別の指標です。",
  ...
  '<li><b>日本の投票支援ツールの方法論。</b>国内の代表的なサービスについて、'
  '政党の立場を誰がどう決め、一致度をどう計算しているかの公開文書を見つけられませんでした。'
  '最も直接的な比較対象なので、優先して確認します。</li>'
  '<li><b>スタンス分類の到達精度。</b>NTCIR QA Lab-PoliInfo のタスク設計は確認しましたが、'
  '賛成／反対／中立分類でどの程度の精度が出ているかの具体的な数値は未確認です。</li>'
  '</ul></div></section>'
...
FOOTER = ("<style>"
  ...
  '<a href="feedback.html">ご意見</a></div>'
