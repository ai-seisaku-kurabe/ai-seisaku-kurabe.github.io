...
import html, json, os, shutil, re, sys, urllib.parse

# 掲載している会期は build_party.py の sessions_for() が唯一の出どころ。
# 会期の一覧を各所に書き写すと、増やしたときに片方だけ古くなる（⑧査読で実際に指摘された）。
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "agents"))
from _sessions import published_sessions

SESSIONS_PUB = published_sessions()                                   # 例: ["217","219","221"]
SESSIONS_JP = "・".join("第%s回" % s for s in SESSIONS_PUB)            # 例: 第217回・第219回・第221回
SESSIONS_DESC = list(reversed(SESSIONS_PUB))                          # 新しい順（一覧・絞り込み用）
...
/* 既定のリンク色。個別に色を指定していないリンクは、これが無いとブラウザ既定
   ...
   (実測: 公開7ページ・221か所がこの状態だった)。要素セレクタなので、
   .topnav a や .ab-cta a.p など個別に指定しているリンクには影響しない。 */
a{color:var(--accent);}
...
.steps{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:0 0 34px;}
@media(max-width:640px){.steps{grid-template-columns:1fr;gap:8px;}}
.step{background:var(--card);border:1px solid var(--line);border-radius:12px;padding:13px 15px;}
.step .sn{font-family:var(--mono);font-size:11px;letter-spacing:.1em;color:var(--accent);}
.step .st{font-family:var(--serif);font-size:15px;font-weight:600;margin:3px 0 2px;}
.step .ss{font-size:12px;color:var(--muted);line-height:1.7;}
"""
...
<div class="wrap">{nav("index.html")}<div class="doc">
  ...
  <div class="steps">
    <div class="step"><div class="sn">STEP 1</div><div class="st">探す</div>
      <div class="ss">政党・政策分野・言葉から、関係する記録を探します。</div></div>
    <div class="step"><div class="sn">STEP 2</div><div class="st">言と行を並べて見る</div>
      <div class="ss">国会での発言（言）と、記名投票の賛否（行）を同じ画面に置きます。</div></div>
    <div class="step"><div class="sn">STEP 3</div><div class="st">原典で確かめる</div>
      <div class="ss">すべての発言・採決に、会議録と投票結果への直接リンクが付いています。</div></div>
  </div>
  <p class="sec">各政党の政策パッケージ</p>
  <p class="sec-s">カードを押すと、その党の全分野の言と行（政党で選ぶ）へ進みます。</p>
  <div class="pgrid">{party_cards}</div>
  <p class="note">出典：国会会議録検索システム／参議院 記名投票結果（{SESSIONS_JP}国会）。
...
FEEDBACK_FORM = ('  <p class="eyebrow">ご意見・お問い合わせ</p>'
  ...
  'このフォームは本文・送信元のページ名・受信時刻しか保存せず、連絡先を受け取る作りになっていません。'
  ...
  # ⑥運用班: 窓口の新着は feedback-count.yml が半日ごとに数える(ご意見=件数、Issue/PR=未対応件数)。
  # 「確認のうえ対応します」と書く以上、どれくらいで気づくのかも書く。実装との一致は
  # verify_content.py の check_intake_watch() が見る(監視を外したら開示も止まる)。
  '・<b>新着に気づくまでの時間について。</b>このフォームとGitHubのIssue・Pull Requestは、'
  '<b>半日ごとに新着の有無を機械的に確認</b>しています（数えるのは件数だけで、本文は取得しません）。'
  '常時見張っているわけではありません。GitHubの定時実行は混み具合でずれることがあり、'
  '実際に数時間遅れた例があります。気づくまでに半日以上かかる場合があるとお考えください。</p>'
  '<div class="fb-thanks" style="margin-top:22px">''<b>権利に関する申出も、この窓口で受け付けます。</b><br>''国会での発言の著作権は、<b>発言した議員ご本人</b>に帰属します。''このサイトは発言の一部を引用し、全文は会議録の原典リンクに委ねる形で掲載しています。''掲載のしかたについて<b>削除・訂正のご要望がある場合</b>は、上のIssueか''リポジトリ経由でお知らせください。確認のうえ対応します'
  '（気づくまでの時間は、上の「新着に気づくまでの時間について」のとおりです）。''詳しくは<a class="src" href="about.html">このサイトについて</a>の「出典と権利」をご覧ください。</div>')
FEEDBACK_JS = FB_TAGS + """
<script>(function(){
...
ONEISSUE=(f'<title>ワンイシュー — 各党が最も重視する一点 ｜ AI政策くらべ</title>\n'
  ...
  '<p class="oi-lede"><b>ただし、ここに挙げた1点は党の全体像ではありません。</b>'
  '研究では、看板政策を1つ見ると有権者は<b>その党の他の政策の立場まで推し量ってしまう</b>こと、'
  '「単一争点の党」という見え方は外からの知覚で<b>実際の党はより多面的</b>であること、'
  '何を重視するかは<b>会期ごとに動く</b>ことが指摘されています'
  '（<a class="src" href="research.html#tab-03">先行研究の03</a>）。'
  'この1点から他の政策を推し量らず、<a class="src" href="guide.html">政党で選ぶ</a>の'
  '分野別の言と行でご確認ください。</p>'
...
ABOUT=(f'<title>このサイトについて（方法論と透明性）｜ AI政策くらべ</title>\n'
  ...
  '<ul class="ab-list">'
  '<li>出典は<a class="src" href="https://kokkai.ndl.go.jp/" target="_blank" rel="noopener">国会会議録検索システム</a>（' + SESSIONS_JP + '国会）。API経由で発言を取得します。</li>'
  ...
  '「政党で選ぶ」では' + SESSIONS_JP + '国会の参院記名投票を並べています。'
  ...
  '運営者が受け取るのは<b>本文・送信元のページ名・受信時刻だけ</b>で、'
...
RESEARCH=(f'<title>先行研究と、この設計の根拠｜ AI政策くらべ</title>\n'
  ...
  + _rs("keep",
    ...
    "<b>もうひとつ、混同されやすい概念があります。</b>顕出性（どの争点を強調するか）と、"
    "<b>争点所有</b>（その争点について、有権者から「この党が得意だ」と見られていること）は別のものです。"
    "争点所有は有権者の評判にかかわる話で、国会の一次情報だけでは測れません。"
    "<b>このサイトは「この党はこの争点に強い／得意だ」とは一切言いません。</b>"
    "ワンイシューは、その党が前面に出している1点を示すだけで、"
    "それをうまく扱えているかどうかの評価は含みません。",
    _cite("Manifesto Project（MARPOR・顕出性理論にもとづく公約分析）", "https://manifesto-project.wzb.eu/")
    + " ／ " + _cite("争点所有（issue ownership・Petrocik 1996 の書誌）", "https://en.wikipedia.org/wiki/Issue_ownership"))
  ...
  + _rs("drop",
    "「この争点は、あとから大政党にも取り込まれた」と示すこと",
    "ニッチ政党の研究は、<b>対抗する大政党が同じ争点を強調しはじめると、その党の「際立ち」は失われる</b>"
    "と論じる。小さな政党が持ち込んだ争点を、大政党があとから取り込む——という動きは、"
    "政党政治の実際によく見られる。これを時系列で示せれば、少数政党の役割は分かりやすくなる。",
    "<b>示しません。</b>理由は2つあります。"
    "①<b>「どちらが先か」を、このサイトのデータでは決められません。</b>"
    "掲載しているのは3会期ぶんで、争点の多くはそれ以前から国会にあります。"
    "手元の窓の中で先に見えたからといって、その党が持ち込んだとは言えません。"
    "②仮に並べれば、読者は<b>「小政党が大政党を動かした」という因果</b>として読みます。"
    "私たちはその因果を確かめる手立てを持っていません。"
    "会期をまたぐ変化そのものは、<a class=\"src\" href=\"guide.html\">政党で選ぶ</a>の会期併記と"
    "<a class=\"src\" href=\"speeches.html\">発言一覧</a>で、読者が直接ご覧になれます。"
    "<b>並べるところまでは私たちがやり、意味づけは読者に返す</b>——という切り分けです。",
    _cite("Meyer &amp; Miller (2015) The niche party concept and its measurement (Party Politics 21-2)",
          "https://pmc.ncbi.nlm.nih.gov/articles/PMC5180693/"))

  + _rs("drop",
    "「能力の主張」と「実績」を並べて、valence 争点を扱うこと",
    "誰も反対しない目標（透明化、手取りを増やす、危機対応）をめぐる競争は、賛否ではなく"
    "<b>どの党が実現できるかという能力の評価</b>で決まる（valence 争点）。"
    "そこで「各党が自分の能力の根拠として何を挙げているか」を、提出法案や役職の経歴と"
    "並べて示す、という設計が考えられる。",
    "<b>並べません。</b>「主張」と「実績」を隣り合わせに置くと、"
    "読者が受け取るのは<b>「言った通りにやれているか」という採点</b>です。"
    "それは私たちがしないと決めている評価そのものになります。"
    "また「何をその党の能力の根拠とするか」を選ぶ時点で、編集の重みが強く入りすぎます。"
    "<b>代わりに、材料を別々のまま置いています。</b>"
    "各党が何を訴えているかは<a class=\"src\" href=\"speeches.html\">発言一覧</a>、"
    "実際にどう投じたかは<a class=\"src\" href=\"votes.html\">採決一覧</a>にあり、"
    "どちらも評価を付けていません。<b>突き合わせる作業は、読者の手に残します。</b>",
    _cite("Stokes (1963) Spatial Models of Party Competition (American Political Science Review 57-2)",
  ...
  + _rs("meas",
    ...
    "残りについて、以前ここには「検討して外したのではなく、多くは検討そのものをしていません」と"
    "書くほかありませんでした。2026年7月に全件のトリアージ（設問候補になるか・ならないなら理由は何かの判定）を導入し、"
    "<b>全" + str(QAUDIT["pool_total"]) + "件に判定を付けて、判定の基準ごと"
    "<a href=\"votes.html\">採決一覧</a>で公開しています。</b>"
    "人事・決算会計・全会一致・予算などは公開している規則で機械的に分類し、"
    "残る議案はAIが下書きした判定を運営者が確定したもので、<b>編集判断を含みます</b>。"
    "選定の基準は、設問を選んだ時点では文書になっていませんでした。"
    "ここに出しているのは事後に洗い出した記録で、当時の議事録ではありません。"
    "<b>Wahl-O-Mat のような多段階の手続きは、いまも踏んでいません。</b>"
    "この開示と判定は問題を解消するものではなく、問題の大きさを見えるようにするものです。"
  ...
  + _rs("open",
    "「どちらでもない」という選択肢が、いくつもの意味を抱え込んでいる",
    "投票支援ツールの設計研究は、設問だけでなく<b>選択肢の作り方</b>も結果を左右すると指摘してきた。"
    "とりわけ中間の選択肢は、「賛成と反対のちょうど中間」「まだ決めていない」「その争点に関心がない」"
    "「判断する情報がない」といった<b>別々の心の状態を、ひとつのボタンに詰め込んでしまう</b>。"
    "これらは政治的にまったく違う状態だが、集計の上では区別がつかない。",
    "「政策で照らす」の「どちらでもない」も、この問題をそのまま抱えています。"
    "<b>選ばれた設問は一致度の計算から外れます</b>（賛成と反対の中間として数えてはいません）。"
    "そのため、多く選ぶほど<b>ごく少ない設問だけで一致度が出る</b>ことになり、割合は不安定になります。"
    "画面では、この扱いを設問の前と結果の両方に明示し、"
    "賛成・反対で答えた設問が3問以下のときは<b>「ごく少ない材料から出ています」と先に警告</b>するようにしました。"
    "<b>ただし、意味の混在そのものは解けていません。</b>"
    "「決めていない」と「関心がない」を分けて聞けば精度は上がりますが、設問数が増え、"
    "答える負担と離脱が増えます。どちらを取るかは未解決です。",
    _cite("Louwerse &amp; Rosema (2014) The design effects of voting advice applications (Acta Politica)",
          "https://link.springer.com/article/10.1057/ap.2013.30"))
  ...
  '分野の網羅ではありません。</li>'
  '<li>調査の<b>生の記録</b>(未確認の記述や、後に誤りと判明した記述もそのまま残したもの)は、'
  'リポジトリの<a class="src" href="https://github.com/ai-seisaku-kurabe/ai-seisaku-kurabe.github.io/tree/main/docs/lit" '
  'target="_blank" rel="noopener">docs/lit</a>で公開しています。'
  '外部AIとの設計相談の記録は<a class="src" href="https://github.com/ai-seisaku-kurabe/ai-seisaku-kurabe.github.io/tree/main/docs/consultations" '
  'target="_blank" rel="noopener">docs/consultations</a>にあります。</li></ul>'
  '<div class="ab-cta">'
...
PRIVACY=(f'<title>プライバシーポリシー ｜ AI政策くらべ</title>'
  ...
  '1件ずつ個別に保存します。</b>受け取るのは本文・送信元のページ名・受信時刻だけで、'
...
def kw_row(el_id, label, placeholder, fields):
    ...
    有権者が最初に持っているのは、政党名でも分野名でもなく**言葉**であることが多い。
    絞り込みのチップとAND で効き、入力のたびに1ページ目へ戻る。

    並び順は日付の新しい順のままにする（関連度で並べ替えない）。同じ条件なら誰が見ても
    同じ順序になるほうが、原典に当たり直すときに確かめやすいため。
    照合は端末の中だけで行い、入力した語はどこへも送らない（アクセス解析を置いていないので、
    どんな語が入力されたかは運営者にも分からない）。
    """
...
NEWS=(f'<title>政策ニュース — 分野別のアーカイブ ｜ AI政策くらべ</title>\n'
  ...
  + kw_row("nwKw", "言葉で探す", "例：物価、賃上げ、防衛", "見出しと媒体名") +
  '</div>'
...
SPEECH_ITEMS.sort(key=lambda x: (x["ses"] == SESSIONS_PUB[0], x["date"]), reverse=False)
...
SP_JS = ("<script>(function(){"
  ...
  "return (fd==='all'||x.domain===fd)&&(fp==='all'||D.pmap[x.party]===fp)&&(fs==='all'||x.ses===fs)&&hit(x);});"
...
SPEECHES = (f'<title>発言一覧 — 党と分野で探す ｜ AI政策くらべ</title>'
  ...
  + "".join(f'<button type="button" class="nw-chip" data-g="s" data-v="{s}">第{s}回</button>'
            for s in SESSIONS_DESC) + '</div>'
  + kw_row("spKw", "言葉で探す", "例：消費税、物価、原発", "発言の引用・議員名・会議名") +
  '</div>'
  '<p class="nw-count"><b id="nwCount">…</b></p>'
  '<div class="nw-list" id="nwList"></div><div class="nw-pager" id="nwPager"></div>'
  f'<p class="note" style="margin-top:30px">出典：国会会議録検索システム（{SESSIONS_JP}国会）。'
...
VOTE_ITEMS.sort(key=lambda x: (SESSIONS_PUB.index(x["ses"]), x.get("date", "")), reverse=True)
# 全件トリアージ(判定表)。生成=agents/triage_votes.py、コード定義=vote_triage_criteria.json、
# 取り決め=agents/TRIAGE_CRITERIA.md。全議案に判定が無ければビルドを止める
# (「全件に判定を付けています」という公開の約束を、欠けたまま公開しないため)。
TRIAGE = json.load(open("vote_triage.json", encoding="utf-8"))
TR_CRIT = json.load(open("vote_triage_criteria.json", encoding="utf-8"))
_tr_by_id = {r["id"]: r for r in TRIAGE["items"]}
for _v in VOTE_ITEMS:
    _r = _tr_by_id.get(_v["id"])
    if not _r:
        raise SystemExit(f"採決 {_v['id']} に判定がありません。agents/triage_votes.py を回してください。")
    _v["tr"] = {k: _r[k] for k in ("code", "note", "rep") if k in _r}
TR_CODES = {c: TR_CRIT["codes"][c]["name"] for c in TR_CRIT["codes"]}
# 判定の基準(コード定義)と件数ファネル。文面は vote_triage_criteria.json から生成し、写しを作らない。
TR_FUNNEL = "".join(
    f'<li><b>{TR_CRIT["codes"][c]["name"]}（{TRIAGE["counts"][c]}件）</b>'
    f'<span class="vt-cdesc">{TR_CRIT["codes"][c]["desc"]}</span></li>'
    for c in TR_CRIT["precedence"] if c in TRIAGE["counts"])
TR_CHIPS = ('<button type="button" class="nw-chip on" data-t="all">すべて</button>' + "".join(
    f'<button type="button" class="nw-chip" data-t="{c}">{TR_CRIT["codes"][c]["name"]} {TRIAGE["counts"][c]}</button>'
    for c in TR_CRIT["precedence"] if c in TRIAGE["counts"]))
VT_JSON = json.dumps({"items": VOTE_ITEMS, "codes": TR_CODES}, ensure_ascii=False)
VT_CSS = NEWS_CSS + """
...
.vt-tri{margin:7px 0 0;font-size:11.5px;line-height:1.7;color:var(--muted);}
.vt-tr{display:inline-block;font-family:var(--mono);font-size:10px;font-weight:700;
  border:1px solid var(--line);border-radius:4px;padding:1px 7px;margin-right:6px;color:var(--muted);}
.vt-tr.tr-USED{color:var(--accent);border-color:var(--accent);}
.vt-trnote{color:var(--muted);}
.vt-crit{border:1px solid var(--line);border-radius:10px;background:var(--card);
  padding:12px 16px;margin:14px 0 0;font-size:12.5px;line-height:1.9;color:var(--muted);}
.vt-crit summary{cursor:pointer;font-weight:700;color:var(--ink);font-size:13px;}
.vt-crit ul{margin:8px 0;padding-left:1.2em;}
.vt-crit li{margin:6px 0;}
.vt-cdesc{display:block;font-weight:400;}
"""
VT_JS = ("<script>(function(){"
  ...
  "var tr=x.tr||{},tn=D.codes[tr.code]||tr.code||'';"
  "var ti=tn?('<p class=\"vt-tri\"><span class=\"vt-tr tr-'+tr.code+'\">判定：'+esc(tn)+'</span>'"
  "+(tr.note?'<span class=\"vt-trnote\">'+esc(tr.note)+'</span>':'')"
  "+(tr.rep?'<span class=\"vt-trnote\">（代表採決：'+esc(tr.rep)+'）</span>':'')+'</p>'):'';"
  ...
  "+'<div class=\"vt-row\">'+ps+'</div>'+ti+'</div>';}).join('');}"
  "document.querySelectorAll('.nw-chip[data-v]').forEach(function(c){c.addEventListener('click',function(){"
  "document.querySelectorAll('.nw-chip[data-v]').forEach(function(x){x.classList.remove('on');});"
  "c.classList.add('on');fs=c.getAttribute('data-v');page=1;render();});});"
  "document.querySelectorAll('.nw-chip[data-t]').forEach(function(c){c.addEventListener('click',function(){"
...
VOTES = (f'<title>採決一覧 — 参議院の記名投票を全件 ｜ AI政策くらべ</title>'
  ...
  '<b>「政党で選ぶ」で各分野に載せているのは、この中から選んだ代表例</b>です。'
  'また、<b>全件に「政策で照らす」の設問候補になるか・ならない理由の判定</b>を付けています'
  '（下の「判定の基準」参照）。</p>'
  ...
  '「—」は、その会期にその党の単独の会派が無く、会派としての賛否が記録に残らない場合です'
  '（統一会派に属していた第217・219回の社民、会派の無かった第217回の参政党・チームみらいなど）。'
  '統一会派の賛否は、その<b>会派全体の結果</b>を、会派名の先頭の党の欄'
  '（例：立憲民主・社民・無所属 → 立憲）に表示しています。</p>'
  '<details class="vt-crit"><summary>判定の基準 — 全' + str(TRIAGE["total"]) + '件を、どう分類したか</summary>'
  '<p>「政策で照らす」の設問は、この一覧の記名投票から作っています。どの採決が設問候補になり、'
  'ならなかったものはなぜかを、全件について判定し、以下の区分で公開します。'
  '<b>どの区分も「重要でない」という意味ではありません</b>。設問の形にできるかどうかの分類であって、'
  '議案の重要度の格付けではありません。</p>'
  '<ul>' + TR_FUNNEL + '</ul>'
  '<p>人事・決算会計・院の運営・全会一致・予算は、公開している件名の規則と投票数から機械的に分類しています。'
  '残る議案の判定はAIが下書きし、運営者が確認して確定したもので、<b>編集判断を含みます</b>。'
  '判定の版：' + TRIAGE["criteria_version"] + '。基準・判定・生成の一式は'
  '<a href="https://github.com/ai-seisaku-kurabe/ai-seisaku-kurabe.github.io/tree/main/tools" '
  'target="_blank" rel="noopener">リポジトリのtools</a>にあり、再現できます。'
  '設問の選び方そのものについては<a href="shindan.html">政策で照らす</a>の'
  '「この11問は、どうやって選んだのか」と<a href="research.html">先行研究</a>の04をご覧ください。</p>'
  '</details>'
  '<div class="nw-filters">'
  '<div class="nw-row"><span class="nw-lbl">会期</span>'
  '<button type="button" class="nw-chip on" data-v="all">すべて</button>'
  + "".join(f'<button type="button" class="nw-chip" data-v="{s}">第{s}回</button>'
            for s in SESSIONS_DESC) + '</div>'
  + '<div class="nw-row"><span class="nw-lbl">判定</span>' + TR_CHIPS + '</div>'
  + kw_row("vtKw", "言葉で探す", "例：予算、年金、防衛", "議案名") +
  '</div>'
  '<p class="nw-count"><b id="nwCount">…</b></p>'
  '<div class="nw-list" id="nwList"></div><div class="nw-pager" id="nwPager"></div>'
  f'<p class="note" style="margin-top:30px">出典：参議院 記名投票結果（{SESSIONS_JP}国会）。'
...
# ---------- 掲載データの範囲（フッターに出す数字） ----------
# 「いつ時点の、どこまでのデータか」を画面から判断できるようにする。
# 数字と日付は**公開している一覧そのもの**（SPEECH_ITEMS / VOTE_ITEMS）から作る。手で書かない。
#
# 日付の名前と根拠を一致させること（取り違えると、開示のつもりが誤情報になる）:
#   - 発言 … 各件が完全な日付（YYYY-MM-DD）を持つので、その最大値＝最も新しい発言の日
#   - 採決 … データが持つのは会期と月日だけで年を持たない。年を推測せず「第221回 7/17」と出す
#   - ページの生成日は出さない。再ビルドしただけで日付が動き、データの新しさを表さないため
#   - ニュース … リポジトリの news.json を GitHub Actions が日次で更新し、HTMLは作り直されない。
#                ビルド時に埋め込むと必ず古くなるので、**実行時に news.json から読む**
SPEECH_N = len(SPEECH_ITEMS)
SPEECH_LAST = max((i.get("date", "") for i in SPEECH_ITEMS), default="")
VOTE_N = len(VOTE_ITEMS)
_vlast = VOTE_ITEMS[0] if VOTE_ITEMS else None          # 会期の新しい順に並べ替え済み
VOTE_LAST = (f'第{_vlast["ses"]}回 {_vlast.get("date", "").lstrip("0").replace("/0", "/")}'
             if _vlast and _vlast.get("date") else "")

DATA_RANGE = ("<b>掲載データの範囲</b>　" + SESSIONS_JP + "国会"
              + f"｜国会発言 {SPEECH_N}件" + (f"（最新 {SPEECH_LAST}）" if SPEECH_LAST else "")
              + f"｜記名投票 {VOTE_N}件" + (f"（最新 {VOTE_LAST}）" if VOTE_LAST else "")
              + '<span id="sfNews"></span>')

# news.json は Actions が更新する（HTMLは作り直されない）ので、日付は実行時に読む。
# updated_utc が無い版の news.json でも壊れないよう、読めた部分だけを出す。
SF_NEWS_JS = ("<script>(function(){var el=document.getElementById('sfNews');if(!el)return;"
  "fetch('news.json').then(function(r){return r.ok?r.json():null;}).then(function(d){"
  "if(!d)return;var mx='';"
  "function scan(a){if(!a)return;for(var i=0;i<a.length;i++){var p=a[i]&&a[i].pub;if(p&&p>mx)mx=p;}}"
  "function walk(o){if(!o)return;for(var k in o){if(Object.prototype.hasOwnProperty.call(o,k))scan(o[k]);}}"
  "walk(d.topics);walk(d.parties);"
  "var t='';if(d.updated_utc){var dt=new Date(d.updated_utc);if(!isNaN(dt.getTime())){"
  "function z(n){return('0'+n).slice(-2);}"
  "t=dt.getFullYear()+'-'+z(dt.getMonth()+1)+'-'+z(dt.getDate())+' '+z(dt.getHours())+':'+z(dt.getMinutes());}}"
  "var s=[];if(t)s.push('最終取得 '+t+'（お使いの端末の時刻）');"
  "if(mx)s.push('記事の最新公開日 '+mx);"
  "if(s.length)el.textContent='｜ニュース '+s.join('・');"
...
FOOTER = ("<style>"
  ...
  ".sf-data{max-width:900px;margin:20px auto 0;padding-top:14px;border-top:1px dashed var(--line);"
  "font-family:var(--sf-mono);font-size:11px;line-height:1.95;color:var(--muted);}"
  ".sf-data b{color:var(--ink);}"
  ".sf-datan{max-width:900px;margin:5px auto 0;font-size:11px;line-height:1.85;color:var(--muted);}"
  ".sf-datan a{color:var(--accent);text-decoration:none;}"
  "</style>"
  ...
  f'（{SESSIONS_JP}国会）。要約は編集判断を含みます。'
  '<b>点数化も格付けもしません。</b>判断は原典リンクで裏取りしてください。</p>'
  '</div>'
  f'<p class="sf-data">{DATA_RANGE}</p>'
  '<p class="sf-datan">件数と日付は、公開している'
  '<a href="speeches.html">発言一覧</a>・<a href="votes.html">採決一覧</a>'
  'のデータそのものから作っています。ニュースの取得は自動で、実行が遅れることがあります。'
  '掲載していない会期・議案があることは<a href="about.html">サイトについて</a>に書いています。</p>'
  '</footer>' + SF_NEWS_JS)
