...
NEWS_JS=("<script>(function(){"
  ...
  "document.getElementById('nwCount').textContent=L.length+' 件';""var cta=document.getElementById('nwCta'),ctaT=document.getElementById('nwCtaT');""if(cta){if(fd==='all'){cta.href='guide.html';""ctaT.textContent='各党が国会で何を論じ、どう投票したかを、原典リンク付きで確かめる →';}""else{cta.href='guide.html?domain='+encodeURIComponent(fd);""ctaT.textContent='「'+fd+'」について、各党が国会で何を論じ、どう投票したかを確かめる →';}}"
