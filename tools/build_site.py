...
def oi_records(full):
    ...
    if not fn or not vk:
        return hd+'</div><p class="oi-none">この党は第217回国会の参議院で会派を構成しておらず、この会期の会派別記名投票記録がありません（第221回の賛否は「政党で選ぶ」のページで確認できます）。</p>'
    rows=[]
...
ABOUT=(f'<title>このサイトについて（方法論と透明性）｜ AI政策くらべ</title>\n'
  ...
  '<li><b>発言（言）は衆議院・参議院の両方</b>から採っています。国会会議録検索システムが両院を収録しているためで、''引用にはどちらの議院・どの委員会かを明記しています。''<b>ただし例外があります。</b>会派名から党を判定しているため、党名を含まない統一会派に属する議員の発言は収集できていません。''現在、衆議院の会派<b>「中道改革連合・無所属」</b>（立憲民主党・公明党などの統一会派。2026年で約3,977件の発言）が''これに当たり、その結果<b>立憲民主党と公明党の「言」は参議院のものに限られています</b>。''「両院から」という以前の記載はこの2党について正確ではありませんでした。訂正しておわびします。''議員名から党を特定する方式への切り替えを検討していますが、統一会派を代表した発言を1党の主張として扱ってよいかという''別の問題があるため、結論が出るまでは現状を開示したうえで収集対象外としています。</li>''<li>一方、<b>採決（行）は参議院のみ</b>です。衆議院の本会議は原則として起立採決で、会派別・個人別の賛否が公式記録に残らないためです。''この<b>衆参の非対称</b>は、データの制約であって編集方針ではありません。</li>''<li>憲法分野は記名投票の議案が無く「行」は該当なしとしています。</li></ul></section>'
...
SPEECHES = (f'<title>発言一覧 — 党と分野で探す ｜ AI政策くらべ</title>'
  ...
  '<p class="nw-note">収集の範囲について：党の判定に会派名を用いているため、'
  '党名を含まない統一会派（衆議院「中道改革連合・無所属」＝立憲民主党・公明党など）の発言は拾えていません。'
  'そのため<b>立憲民主党と公明党は、参議院での発言のみ</b>が並びます。'
  '<a href="about.html">収集ルールと、この制約の詳細</a></p>'
  '<div class="nw-filters">'
  '<div class="nw-row"><span class="nw-lbl">分野</span>'
  ...
  '<p class="note" style="margin-top:30px">出典：国会会議録検索システム（第217回・第221回国会）。'
