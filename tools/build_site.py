...
def oi_records(full):
    ...
    if not fn or not vk:
        return hd+'</div><p class="oi-none">参政党は当会期の参議院で会派を構成しておらず、会派別の記名投票記録がありません（立場は上の発言でご確認ください）。</p>'
    rows=[]
...
ABOUT=(f'<title>このサイトについて（方法論と透明性）｜ 政策くらべ</title>\n'
  ...
  '<li>衆議院は個人別の賛否が記録に残らないため、<b>参議院の記名投票のみ</b>を扱います。憲法分野は記名投票の議案が無く「行」は該当なしとしています。</li></ul></section>'
  ...
  'データが無い所は捏造せず空欄にします（例：参政党は会派未結成のため会派別の採決記録がありません）。</p></div>'
...
NEWS_JS=("<script>(function(){"
  ...
  "function esc(s){return String(s).replace(/[&<>\"]/g,function(c){return({'&':'&amp;','<':'&lt;','>':'&gt;','\\\"':'&quot;'}[c]);});}"
  "var ALL=[],fd='all',fp='all';"
  "function render(){var L=ALL.filter(function(x){"
  "return (fd==='all'||(x.d||[]).indexOf(fd)>=0)&&(fp==='all'||(x.p||[]).indexOf(fp)>=0);});"
  "document.getElementById('nwCount').textContent=L.length+' 件';"
  "var root=document.getElementById('nwList');"
  "if(!L.length){root.innerHTML='<div class=\"nw-empty\">条件に合う見出しがありません。</div>';return;}"
  ...
  "c.classList.add('on');"
  "if(g==='d'){fd=c.getAttribute('data-v');}else{fp=c.getAttribute('data-v');}render();});});"
  "fetch('news_archive.json').then(function(r){return r.json();}).then(function(d){"
...
NEWS=(f'<title>政策ニュース — 分野別のアーカイブ ｜ 政策くらべ</title>\n'
  ...
  '</div>'
  '<p class="nw-count"><b id="nwCount">…</b> ／ <span id="nwUpd">読み込み中</span></p>'
