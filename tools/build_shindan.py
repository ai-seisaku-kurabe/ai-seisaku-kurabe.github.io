# -*- coding: utf-8 -*-
"""政党相性診断 v1.0
第1問=ワンイシュー共感、第2問以降=政策整合。各党の賛否は設問ごとに示した参院採決と国会発言に基づく
（どの会期かは POLICY に貼った採決URLが唯一の出どころ。SESSIONS_JP がそれを読んで文面を作る）。
...
for _i,(_b,_u) in enumerate([('該当する記名採決が無いため、各党の公約と国会での発言にもとづく判定です。', ''), ('各党の公約と国会での発言にもとづく判定です。関連する採決はありますが、賛否だけでは賛成・反対の理由を一意に 判断 できないため、採決のみを根拠とはしていません。', ''), ('該当する記名採決が無いため、各党の公約と国会での発言にもとづく判定です。', ''), ('該当する記名採決が無いため、各党の公約と国会での発言にもとづく判定です。', ''), ('各党の公約と国会での発言にもとづく判定です。関連する採決として第221回の憲法改正手続法（国民投票法）改正がありますが、これは改正の手続をどう整えるかについての賛否で、憲法改正そのものへの賛否とは別のため、採決を判定の根拠にはしていません。', 'https://www.sangiin.go.jp/japanese/touhyoulist/221/221-0724-v001.htm'), ('各党の公約と国会での発言にもとづく判定です。予算への賛否は「規模が大きすぎる／小さすぎる」の両方の理由から反対が生じるため、採決のみを根拠とはしていません。', ''), ('第221回国会・日比ACSA（物品役務相互提供協定）の承認に対する各党の賛否にもとづく判定です。', 'https://www.sangiin.go.jp/japanese/touhyoulist/221/221-0619-v001.htm'), ('該当する記名採決が無いため、各党の公約と国会での発言にもとづく判定です。', ''), ('第221回国会・入管法改正への賛否にもとづく判定です。政府の趣旨説明は「不法残留等を企図する者の入国を防止することにより厳格な出入国管理を実現する」「在留外国人にも相応の負担を求める」と述べており、規制を強める内容と確認しました。ただし公明党は、賛成しつつ低所得の外国人の手数料免除を求める修正案を提出しているため、中立としています。', 'https://www.sangiin.go.jp/japanese/touhyoulist/221/221-0529-v007.htm'), ('各党の公約にもとづく判定です。関連する採決として第221回の高等学校等就学支援金法改正がありますが、参政党とれいわ新選組はこれに反対票を投じています。討論が行われず反対の理由を確認できないため、採決を判定の根拠にはしていません。判定と投票行動が食い違う例として、採決結果も併せてご確認ください。', 'https://www.sangiin.go.jp/japanese/touhyoulist/221/221-0331-v006.htm')]):
    POLICY[_i]["basis"]=_b; POLICY[_i]["basis_url"]=_u

# 判定が何にもとづくかは、リンクの有無ではなく明示する。採決へのリンクを貼っていても
# 「採決は根拠にしていない」と本文で断っている設問（憲法改正・教育無償化）があり、
# リンクの有無で数えると、公約・発言ベースの設問まで採決ベースとして数えてしまう
# （⑨の設問の来歴 state/question_audit.json と research.html の数字がそれだった）。
for _i,_k in enumerate(["公約・発言","公約・発言","公約・発言","公約・発言","公約・発言",
                        "公約・発言","採決","公約・発言","採決","公約・発言"]):
    POLICY[_i]["basis_kind"]=_k

# このページが賛否の根拠にしている会期。設問に貼った参院採決のURLから読む
# （会期名を文面に手で書くと、設問を入れ替えたときに断り書きだけが古いまま残る。
#   このページの脚注は3会期を掲載する今も「第217回国会」のままだった）。
_SES = sorted({m for q in POLICY for m in re.findall(
    r"/touhyoulist/(\d{3})/",
    (q.get("basis_url") or "") + " " + " ".join(u for _, u in q.get("links", [])))})
SESSIONS_JP = "・".join("第" + s + "回" for s in _SES) + "国会"
...
# 記名投票の全件トリアージ(agents/triage_votes.py)。「多くは検討そのものをしていません」を
# 解消した実数をここに流し込む(本文に手で数字を書かない)。無ければビルドを止める。
_tri = json.load(open("vote_triage.json", encoding="utf-8"))
HOWPICKED = (
  ...
  '<li><b>選ばなかった争点：</b>掲載している会期の記名投票は全' + str(_tri["total"])
  + '件あり、設問の根拠・参考に使ったのは' + str(_tri["counts"].get("USED", 0)) + '件です。'
  '以前ここには「多くは検討そのものをしていません」と書いていましたが、現在は'
  '<b>全件に「設問候補になるか・ならないなら理由は何か」の判定を付け、'
  '<a class="src" href="votes.html">採決一覧</a>で判定の基準ごと公開しています</b>。'
  '判定は機械的・中立的なものではなく、編集判断を含みます。</li>'
...
function render(res, pick, answered){
  ...
  // 材料が少ないときは、割合より先にそれを言う。「どちらでもない」は計算から外れるため、
  // 多く選ぶと、ごく少ない設問だけで高い一致度が出てしまう（読者が誤解しやすい最大の点）。
  let thin='';
  if(answered>0 && answered<=3){
    thin='<p class="tienote"><b>賛成・反対で答えた設問が'+answered+'問しかありません。</b>'
      +'「どちらでもない」は計算から外れるため、この一致度は<b>ごく少ない材料</b>から出ています。'
      +'割合の大小はあまり意味を持ちません。答えられる設問を増やすか、'
      +'<a class="src" href="guide.html">政党で選ぶ</a>で各党の言と行を直接ご覧ください。</p>';
  }
  ...
  const html_ = ''
   ...
   +'<h3>あなたの回答との一致度（'+answered+'問に回答）</h3><div class="rank">'+rows+'</div>'+thin+tie+why+div+'</div>'
   +'<div class="rcard"><span class="rlabel">この照合の限界</span>'
   +'<p class="caveat"><b>これは断定ではなく出発点です。</b>各党の賛否は、__SESSIONS__の参院採決・国会発言や各党の公約にもとづく編集判断で、'
   +'「中立/不明(0)」も含みます。「◎ 特に重視」を選んだ争点は3倍で計算しています。'
   +'<b>あなたが「どちらでもない」を選んだ設問と、その党の立場が「中立/不明」の設問は、'
   +'どちらも計算から外れます</b>（中間として数えてはいません）。'
   +'「どちらでもない」には、まだ決めていない・その争点を重視していない・情報が足りない・'
   +'本当に中間だ、といった別々の意味が混ざりますが、ここでは区別していません。'
   +'また、この照合は<b>賛成・反対で答えられる争点</b>しか扱えません。'
   +'誰も反対しない目標（手取りを増やす、政治の透明化など）の実現力や優先度で競う党の中心的な訴えは、ここには表れません。'
   +'その強調の度合いを順位や割合で示すことは、私たち自身の「点数化・格付けをしない」原則に触れるため行っていません。'
   +'各党がどの争点をどれだけ扱っているかは、<a class="src" href="speeches.html">発言一覧</a>の原文でご確認ください'
   +'（原文を読むことは大切ですが、それも賛否の代わりにはなりません）。'
...
<div class="wrap"><div class="doc">
  ...
  <b>第2問以降</b>は主要政策への賛否です。各党の立場は<b>{SESSIONS_JP}の参院採決・国会発言</b>に基づきます。何にもとづく判定かは、設問ごとに個別に記載しています。
  ...
  ただし<b>「どちらでもない」を選んだ設問は、一致度の計算から外れます</b>（賛成と反対の中間として数えるのではありません）。
  そのため多く選ぶほど、少ない設問だけで一致度が出ることになります。
  ...
  <p class="caveat" style="margin-top:20px">
    ...
    <a class="src" href="https://www.sangiin.go.jp/japanese/touhyoulist/217/vote_ind.htm" target="_blank" rel="noopener">参議院 記名投票結果</a>（{SESSIONS_JP}）。
    参政党は会派未結成のため一部の採決データがなく、該当設問は中立扱いです。
  </p>
</div></div>
<script>{JS.replace("__DATA__", DATA_JSON).replace("__SESSIONS__", SESSIONS_JP)}</script>'''
