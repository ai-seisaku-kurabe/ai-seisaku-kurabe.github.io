# -*- coding: utf-8 -*-
"""政党相性診断 v1.0
第1問=ワンイシュー共感、第2問以降=政策整合。各党の賛否は第217回国会の参院採決・国会発言に基づく。
...
POLICY = [
 {"q":"消費税は引き下げる（または廃止する）べきだ",
  "stance":{"自由民主党":-1,"立憲民主党":0,"日本維新の会":0,"国民民主党":1,"公明党":0,"日本共産党":1,"れいわ新選組":1,"参政党":1},
 ...
 {"q":"憲法改正（緊急事態条項・議員任期延長など）を進めるべきだ",
  "stance":{"自由民主党":1,"立憲民主党":-1,"日本維新の会":1,"国民民主党":0,"公明党":0,"日本共産党":-1,"れいわ新選組":-1,"参政党":1},
 ...
 {"q":"財政は健全化（PB黒字化・歳出抑制）を積極財政より優先すべきだ",
  "stance":{"自由民主党":1,"立憲民主党":0,"日本維新の会":1,"国民民主党":0,"公明党":0,"日本共産党":-1,"れいわ新選組":-1,"参政党":-1},
 ...
 {"q":"外国人の受け入れは、もっと規制・抑制すべきだ",
  "stance":{"自由民主党":0,"立憲民主党":-1,"日本維新の会":1,"国民民主党":0,"公明党":-1,"日本共産党":-1,"れいわ新選組":-1,"参政党":1},
  ...
  "links":[("論点整理（NRI・中立の分析）","https://www.nri.com/jp/media/column/kiuchi/20250710.html"),
 ...
 {"q":"高校・大学まで、教育の無償化を進めるべきだ",
  "stance":{"自由民主党":0,"立憲民主党":1,"日本維新の会":1,"国民民主党":1,"公明党":1,"日本共産党":1,"れいわ新選組":1,"参政党":1},
...
function render(res, pick, answered){
  ...
  let rows = res.map((r,idx)=>{
    const isTop = top && r.p.full===top.p.full && r.pct!==null;
  ...
  if(top){
    ...
    if(matched.length){
      ...
      why='<p class="matchwhy"><b>'+esc(tp.short)+'</b>とは、'+names.join('・')
  ...
  const html_ = ''
   ...
   +'<h3>あなたの回答との一致度（'+answered+'問に回答）</h3><div class="rank">'+rows+'</div>'+why+div+'</div>'
   +'<div class="rcard"><span class="rlabel">この照合の限界</span>'
   +'<p class="caveat"><b>これは断定ではなく出発点です。</b>各党の賛否は、第217回国会の参院採決・国会発言や各党の公約にもとづく編集判断で、'
   +'「中立/不明(0)」も含みます。「◎ 特に重視」を選んだ争点は3倍で計算しています。'
   +'必ず<a class="src" href="https://claude.ai/code/artifact/f8fbacb6-7efb-4f1e-b86f-7c556639379b" target="_blank" rel="noopener">政党で選ぶ（比例区ガイド）</a>で'
...
<div class="wrap"><div class="doc">
  ...
  <b>第2問以降</b>は主要政策への賛否です。各党の立場は<b>第217回国会の参院採決・国会発言</b>に基づきます。
