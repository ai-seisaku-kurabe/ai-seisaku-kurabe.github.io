...
open("ss_speech_candidates.txt","w",encoding="utf-8").write("\n".join(out))
...
SK=["年金","医療","介護","保険","子ども","児童","障害","健康","雇用","生活保護","社会保障","こども","看護","労働"]
ss=[(h,n) for h,n in bills if any(k in n for k in SK)]
open("ss_bills.txt","w",encoding="utf-8").write("\n".join(f"{h}  ||  {n}" for h,n in ss))
print("ss bills:",len(ss))
