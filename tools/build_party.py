...
# 同じ場所を import の探索先にも足しておく（agents/ から起動されたときに contrast が見つからない）。
...
# 憲法は記名投票にかかる議案が少なく「行」がほぼ空になるため最後。
DOMAIN_ORDER = [
...
BILLS221 = {
 ...
 # 憲法はこれまで「行」が空だったが、第221回の7/24に憲法改正手続法（国民投票法）の
 # 改正が本会議の記名投票にかかった。データがある以上、空欄のままにはしない。
 # この会期でこの分野の記名投票はこの1件だけで、選定の余地は無い。
 "憲法": [("221-0724-v001","改正手続法")],
}
...
# 「話題の配分」の党タブ表示（旧 saliency_block）は憲法5条（ランキング禁止）に触れるため撤去。
# 順位・割合を出す集計スクリプト（旧 agents/audit_saliency.py）ごと削除した（⑧査読 chunk A/B）。
...
def votes_block(full, dname, entry, votes, labels):
    ...
    for ses, vk, vv, ll in sessions_for(full, dname, entry, votes, labels):
        ...
        if note:
            ...
            # 会期を跨いで「議案が無い」と断定しない。第221回7/24に憲法改正手続法の
            # 記名投票があり、断定はもう事実と食い違う（第217回・第219回には無い）。
            reason = "この会期には、本会議の記名投票にかかる議案がありません（憲法審査会は討議の場です）"
        elif not vk:
...
# 社会保障は憲法と理由が違う。憲法は発言そのものが確認できなかったが、社会保障は
# 記録はあっても代表発言として取り上げられるものが乏しく、編集判断で見送っていた
# （build_guide.py の SOCIAL の注記）。理由を取り違えて書かない。
LATE_CARDS = {
    ("参政党", "憲法"): "この会期の憲法審査会で、この党の議員の発言を確認できませんでした",
    ("参政党", "社会保障"): "この会期は、この分野で取り上げられる代表発言が乏しく、編集判断で掲載を見送りました",
}
...
# 「点数化・格付け・ランキングを出力しない」に反すると指摘。妥当なので、表示も、順位・割合を出す
# 集計スクリプト（audit_saliency.py）も取り下げた。6分野に収まらない争点があるという弱点は、
# research.html で順位化せず言葉で開示する。
...
for i,(full,short) in enumerate(PARTIES):
    ...
    panes.append(f'<section class="pane" data-pane="{i}" '
                 f'style="--pc:{PARTY_COLOR[full]};--pc-on:{pc_on(PARTY_COLOR[full])};'
                 f'--pc-tl:{contrast.readable_on(PARTY_COLOR[full], "#e6e9f4")};'
                 f'--pc-td:{contrast.readable_on(PARTY_COLOR[full], "#222a40")}"'
...
.oi-label{ display:block; font-family:var(--mono); font-size:11.5px; letter-spacing:.06em;
  color:var(--pc-tl,var(--pc,var(--accent))); margin-bottom:8px; font-weight:700; }
...
.oi-more{ display:inline-block; margin-top:10px; font-family:var(--mono); font-size:12.5px;
  font-weight:600; color:var(--pc-tl,var(--pc,var(--accent))); text-decoration:none;
  border:1px solid var(--pc-tl,var(--pc,var(--accent)));
...
.clip.on{ color:var(--pc-tl,var(--pc,var(--accent))); }
/* 暗い地では、党の色を明るい側へ寄せた値に切り替える（明るい地では暗い側へ寄せてある）。
   党の色そのもの（--pc）は変えない。背景や左罫にはそのまま使う。 */
@media(prefers-color-scheme:dark){
  .oi-label,.clip.on{ color:var(--pc-td,var(--pc,var(--accent))); }
  .oi-more{ color:var(--pc-td,var(--pc,var(--accent))); border-color:var(--pc-td,var(--pc,var(--accent))); }
}
:root[data-theme="dark"] :is(.oi-label,.clip.on){ color:var(--pc-td,var(--pc,var(--accent))); }
:root[data-theme="dark"] .oi-more{ color:var(--pc-td,var(--pc,var(--accent)));
  border-color:var(--pc-td,var(--pc,var(--accent))); }
:root[data-theme="light"] :is(.oi-label,.clip.on){ color:var(--pc-tl,var(--pc,var(--accent))); }
:root[data-theme="light"] .oi-more{ color:var(--pc-tl,var(--pc,var(--accent)));
  border-color:var(--pc-tl,var(--pc,var(--accent))); }
...
<div class="wrap"><div class="doc">
  ...
  <p class="note">
    ...
    行＝参議院 記名投票の会派別賛否（{VOTE_INDEX_LINKS}の{SESSIONS_COUNT}会期を併記）。憲法分野は本会議の記名投票にかかる議案が少なく、第221回の憲法改正手続法（国民投票法）改正の1件だけを載せています（第217回・第219回は該当なし）。<br>
