...
査読は毎回は回さない（憲章「いつ査読にかけるか」）。溜めてから回す運用にすると
**どこまで査読済みかが分からなくなる**のが唯一の実害なので、査読のたびに
`state/reviews/LAST_REVIEWED` に「ここまで見た」コミットを記録し、
次回は既定でその続きから査読する。

使い方:
    python agents/ask_reviewers.py                    # 前回の査読の続きから（無ければ main との差分）
    python agents/ask_reviewers.py --paths guide.html
    python agents/ask_reviewers.py --base HEAD~1
    python agents/ask_reviewers.py --since-merge-base # 前回の記録を使わず、main との分岐点から
    python agents/ask_reviewers.py --unreviewed       # 未査読の範囲を表示するだけ
...
MARKER = os.path.join(RECORDS, "LAST_REVIEWED")   # ここまで査読した、というコミット


def head_sha():
    return mrr.git("rev-parse", "HEAD", check=False).strip()


def read_marker():
    """前回の査読が「ここまで見た」コミット。使えなければ None。"""
    ...
    for line in open(MARKER, encoding="utf-8"):
        ...
        if line and not line.startswith("#"):
            sha = line.split()[0]
            # 履歴を書き換えた後などで、そのコミットが無いことがある
            if not mrr.git("rev-parse", "--verify", "--quiet", sha + "^{commit}",
                           check=False).strip():
                return None
            return sha
    return None
...
def write_marker(sha, stamp):
    os.makedirs(RECORDS, exist_ok=True)
    with open(MARKER, "w", encoding="utf-8") as f:
        f.write("# ⑧査読班が「ここまで査読した」コミット。次回はこの続きから査読する。\n"
                "# 手で書き換えないこと（未査読の範囲が分からなくなる）。\n")
        f.write(f"{sha} {stamp}\n")


def unreviewed_range():
    """未査読のコミット一覧（marker..HEAD）。marker が無ければ None。"""
    m = read_marker()
    if not m:
        return None, []
    log = mrr.git("log", "--oneline", f"{m}..HEAD", check=False).strip()
    return m, [l for l in log.split("\n") if l]
...
def main():
    ...
    ap.add_argument("--since-merge-base", action="store_true",
                    help="前回の査読の記録を使わず、main との分岐点から査読する")
    ap.add_argument("--unreviewed", action="store_true",
                    help="未査読の範囲を表示するだけ（送らない）")
    a = ap.parse_args()

    marker, pending = unreviewed_range()
    if a.unreviewed:
        if not marker:
            print("前回の査読の記録がありません（state/reviews/LAST_REVIEWED が無い）。")
            return 0
        print(f"前回の査読: {marker[:7]}")
        print(f"未査読のコミット: {len(pending)} 件")
        for l in pending:
            print("  " + l)
        return 0

    # 既定は「前回の査読の続きから」。溜めてから回す運用で取りこぼさないため。
    base = a.base
    if not base and not a.since_merge_base and marker and pending:
        base = marker
        print(f"前回の査読（{marker[:7]}）の続きから査読します。未査読 {len(pending)} コミット。")
    base = mrr.base_ref(base)
    units = mrr.changed_files(base, a.paths)
    if not units:
        print("差分がありません。査読するものがありません。")
        if marker and not pending:
            print(f"（{marker[:7]} まで査読済みで、そのあとのコミットはありません）")
        return 0
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write(f"査読した範囲: `{base}`..`{head[:7]}`"
                + (f"（未査読だった {len(pending)} コミット）" if pending else "") + "\n\n")
        if pending:
            f.write("この回で査読したコミット:\n\n"
                    + "".join(f"- {l}\n" for l in pending) + "\n")
        f.write("対象ファイル:\n\n" + "".join(f"- `{p}`\n" for p, _ in units) + "\n")
        f.write("| AI | 判定 |\n|---|---|\n")
    ...
    # 「ここまで査読した」を残す。これが無いと、溜める運用で未査読の範囲が分からなくなる。
    # 範囲を絞って回した回（--paths）は全体を見ていないので、印は進めない。
    if a.paths:
        print("※ --paths で範囲を絞ったため、査読済みの印は進めません"
              "（全体を見たわけではないため）。")
    else:
        write_marker(head_sha(), stamp)
        print(f"査読済みの印を {head_sha()[:7]} に進めました"
              "（次回はこの続きから。state/reviews/LAST_REVIEWED）")
